...
function getAdminQuotationPermissions() {
  return {
    ...quotationPermissionDefaultState,
    show_new_button: true,
    save_edits: true,
    show_linked_new_requirement: true,
    editable_fields: getQuotationEditableFieldKeys(),
  }
}

function normalizeQuotationPermissions(rawPermissions = {}, profile = currentUserProfile) {
  if (profile?.role === 'admin') {
    return getAdminQuotationPermissions()
  }

  const source = rawPermissions && typeof rawPermissions === 'object' ? rawPermissions : {}
  const normalized = { ...quotationPermissionDefaultState }
  const editableKeys = new Set(getQuotationEditableFieldKeys())

  Object.keys(quotationPermissionDefaultState).forEach((key) => {
  ...
  if (!normalized.access) {
    normalized.show_new_button = false
    normalized.show_edit_button = false
    normalized.show_eye_button = false
    normalized.open_edit_modal = false
    normalized.save_edits = false
    normalized.show_linked_detail_button = false
    normalized.show_linked_new_requirement = false
    normalized.show_linked_columns_button = false
    normalized.show_linked_export_excel_button = false
    normalized.show_linked_share_button = false
  }

  if (!normalized.show_edit_button) {
    normalized.open_edit_modal = false
    normalized.save_edits = false
  }

  if (!normalized.open_edit_modal) {
    normalized.save_edits = false
  }

  if (!normalized.save_edits) {
    normalized.editable_fields = []
  }

  if (normalized.save_edits && !normalized.editable_fields.length) {
    normalized.editable_fields = getQuotationEditableFieldKeys()
  }
...
function getCurrentQuotationPermissions(profile = currentUserProfile) {
  const rawPermissions = profile?.metadata?.permissions?.quotations || {}
  return normalizeQuotationPermissions(rawPermissions, profile)
}
...
function setActiveView(viewKey) {
  if (viewKey === 'quotations-log' && currentUserProfile && !userCanAccessQuotations()) {
    viewKey = 'dashboard'
  }
...
function renderAdminPermissionsModal() {
  ...
  const permissions = getCurrentQuotationPermissions(activePermissionsProfile)
  const editableFields = new Set(permissions.editable_fields)
  const canConfigureFields = permissions.save_edits

  if (adminPermissionsTitle) {
    adminPermissionsTitle.textContent = `Permisos de ${activePermissionsProfile.full_name || activePermissionsProfile.email || 'usuario'}`
  }
  ...
  if (adminPermissionsSummary) {
    adminPermissionsSummary.innerHTML = `
      ...
      <article class="rq-summary__card"><span>Modo</span><strong>${permissions.save_edits ? 'Edición controlada' : 'Lectura controlada'}</strong></article>
    `
  }

  if (adminPermissionsGeneral) {
    adminPermissionsGeneral.innerHTML = quotationPermissionGeneralOptions
      .map((option) => buildPermissionToggleMarkup(option, permissions[option.key]))
      .join('')
  }

  if (adminPermissionsLinked) {
    adminPermissionsLinked.innerHTML = quotationPermissionLinkedOptions
      .map((option) => buildPermissionToggleMarkup(option, permissions[option.key]))
      .join('')
  }

  if (adminPermissionsFields) {
    adminPermissionsFields.innerHTML = getQuotationEditableFieldKeys()
      .map((fieldKey) => {
...
function openAdminPermissionsModal(profile) {
  ...
  updateAdminPermissionsStatus('Ajusta los permisos del módulo Log de Cotizaciones para este usuario.', 'info')
...
function collectAdminQuotationPermissions() {
  const formData = new FormData(adminPermissionsForm)
  const nextPermissions = {
    ...quotationPermissionDefaultState,
    editable_fields: formData.getAll('editable_fields').map((value) => String(value)),
  }

  quotationPermissionGeneralOptions.forEach((option) => {
    nextPermissions[option.key] = formData.get(option.key) === 'on'
  })

  quotationPermissionLinkedOptions.forEach((option) => {
    nextPermissions[option.key] = formData.get(option.key) === 'on'
  })

  return normalizeQuotationPermissions(nextPermissions, activePermissionsProfile)
}

async function saveAdminQuotationPermissions() {
  if (!supabaseClient || currentUserProfile?.role !== 'admin' || !activePermissionsProfile) {
    return
  }

  const nextPermissions = collectAdminQuotationPermissions()
  const nextMetadata = {
    ...
    permissions: {
      ...
      quotations: nextPermissions,
    },
  }

  updateAdminPermissionsStatus('Guardando permisos del usuario...', 'info')
...
function renderRequirementModalExplorer() {
  ...
  requirementsExplorerContent.innerHTML = `
    <div class="requirements-explorer requirements-explorer--detail">
      ...
      <div class="requirements-explorer__toolbar">
        <div class="requirements-explorer__toolbar-group">
          <button class="ghost-button ghost-button--accent" type="button" data-action="new-modal-requirement">Nuevo requerimiento</button>
          <button class="ghost-button ghost-button--accent" type="button" data-action="add-existing-resource">${requirementModalResourcePickerOpen ? 'Ocultar catálogo' : 'Agregar recurso'}</button>
          <button class="ghost-button ghost-button--soft" type="button" data-action="toggle-modal-columns">Columnas</button>
          <button class="ghost-button ghost-button--soft" type="button" data-action="download-rq-html">Descargar HTML</button>
          <button class="ghost-button ghost-button--soft" type="button" data-action="export-rq-excel">Exportar Excel</button>
          <button class="ghost-button ghost-button--soft" type="button" data-action="export-rq-pdf">Exportar PDF</button>
          <button class="ghost-button ghost-button--soft" type="button" data-action="print-rq">Imprimir</button>
          <button class="ghost-button ghost-button--soft" type="button" data-action="share-rq">Compartir</button>
        </div>
...
adminPermissionsForm?.addEventListener('change', () => {
  ...
  const stagedPermissions = collectAdminQuotationPermissions()
...
adminPermissionsForm?.addEventListener('submit', async (event) => {
  event.preventDefault()
  await saveAdminQuotationPermissions()
})
